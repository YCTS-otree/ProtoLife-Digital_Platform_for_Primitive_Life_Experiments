# ProtoLife – 数字原始生命实验平台

ProtoLife 旨在验证在有限二维世界中构建具备代谢、感知、行动能力的数字生命体并让其通过强化学习 / 演化形成自主策略的可行性。本仓库提供环境、策略、遗传、通信、记录等模块的初步框架，便于快速迭代实验。

## 核心设计要点
- **纯规则驱动的物理/生理法则**：环境负责移动、碰撞、能量代谢、食物/毒素交互、死亡与再生等硬规则，不写死智能策略。
- **多阶段实验开关**：通过配置切换生存、繁衍、战斗、通信、环境改造等机制，便于逐步演进实验复杂度。
- **压缩日志与回放**：地图编码为单字节单元，可在运行时或离线重放实验过程，支持地图存档导入导出。
- **GPU 优先的向量化并行**：建议在单卡 V100 上并行多个环境与个体，减少 Python 循环。

## 目录结构
```
config/
  default.yaml                # 通用默认配置，可作为基底
  PyTorch_Profiler.yaml       # 按需性能分析的步数、调度、强度与报告配置
  phase0_survival.yaml        # 生存阶段示例（可复制到 model/<name>/<name>.yaml 使用）
model/
  demo_model/                 # 单个模型/实验的归档目录
    demo_model.yaml           # 仅此一份主配置（建议由 config/ 复制后修改）
    checkpoint/               # 训练 checkpoint（#N_...env/model/optim_step_xxx.pth）
    log/                      # 以有效模拟编号命名的 map.log / agents.jsonl
    config/                   # 训练时自动保存的 merge 后配置快照
protolife/
  env.py                      # 网格环境与规则实现
  encoding.py                 # 地图压缩编码
  agents.py                   # 个体状态结构与批量管理
  policy.py                   # 策略与价值网络
  rewards.py                  # 行为奖励配置
  genetics.py                 # 繁衍与变异逻辑
  communication.py            # 消息接口
  logger.py                   # 实验记录
  replay.py                   # 回放工具
  config_loader.py            # YAML 配置管理
  utils/
    cuda_utils.py
    seed_utils.py
    schedulers.py
scripts/
  train_phase0.py
  map_editor.py               # 命令行地图编辑器
  plot_agent_count_v1.py      # 多 JSONL 独立绘制个体数量曲线
  compact_checkpoints.py      # 迁移并校验旧 full checkpoint
  clean_invalid_training_files.py # 清理“无效_”训练产物
  watch_agent_count_live.py   # 实时监控 JSONL 个体数量变化
maps/
  default_map.hex             # 简单示例地图（8×8 全空）
```

## 快速开始（可直接运行的最小 Demo）
> 当前仓库提供的是“可跑通流程”的最小原型：环境、策略与配置均为占位实现，但可以直接执行一轮前向推理与环境步进，用于验证依赖安装和配置是否正确。

1. **准备 Python 环境**
   - 建议 Python 3.9+，可用 `python -m venv .venv && source .venv/bin/activate` 创建虚拟环境。
   - 安装依赖（CPU 版本示例）：`pip install torch pyyaml`。
   - 若需要 GPU/CUDA，请根据显卡与 CUDA 版本替换为官方给出的 `pip install torch==<ver>+cu118 -f https://download.pytorch.org/whl/torch_stable.html` 或直接使用给出的 `pip_CUDA121.bat` ，这是绝大部分搭载较新的NVIDIA GPU都能使用的一键安装脚本。
   - 中国朋友请记得先切换pip清华源，方法自行搜索。

2. **运行基本 Demo**
   tip:请在项目根目录运行
   ```bash
   # 初次使用
   python -m scripts.train_phase0 --config .\config\default.yaml
   ```
   按照指引对模型命名后，训练就开始了。如果需要修改参数，请先Ctrl+C停止代码运行，然后到 `model/<model_name>/<model_name>.yaml` 中更改。相关参数的注释在config/default.yaml
   ```bash
   # 再次训练
   python -m scripts.train_phase0 --model-dir model/<model_name>
   ```
   预期输出：
   - 打印训练进度和速率
   - 显示模型实时画面(如果开启了"实时渲染"的话)
   - 打印一次环境步进后的平均奖励
   
   仅提供 `--model-dir` 时，脚本会，默认从中加载所有模型，如果提供 `--config` 则会使用传入参数覆盖 `model/<model_name>` 
   如果不提供 `--config`，脚本会默认尝试加载 `model/<model_name>/<model_name>.yaml`，并在每次训练开始时打印所使用的模型目录与配置文件的完整路径，便于确认当前实验的来源与产出位置。

3. **配置文件讲解与自定义**
   所有 YAML 位于 `config/`，推荐以 `config/default.yaml` 为基础：
   - `world`：地图尺寸、随机地图中食物/毒素密度、随机种子、外部地图文件（示例：`height: 64`, `width: 64`, `map_file: maps/default_map.hex`）。如果提供了 `map_file` 且文件存在，会优先加载该地图，否则回退到随机/空白地图并按 `food_density`/`toxin_density` 撒点资源。
     Windows 路径请使用正斜杠（例如 `G:/maps/demo.hex`）或在 YAML 中写成双反斜杠（`G:\\maps\\demo.hex`），避免未转义的反斜杠导致解析错误。
     `food_lifetime` 控制食物腐败时间（默认 `2048` step，设为 `0` 关闭）；新生成食物会独立从 0 开始计龄，年龄会随 checkpoint 保存。
   - `agents`：每个环境的个体数量、初始能量等（示例：`per_env: 4`）。
   - `model`：
     - `use_cnn`: 默认开启，环境会为每个体裁剪 `(2r+1)×(2r+1)` 的局部网格并拆成多通道（墙/可建造区/食物/毒素/资源/周围其他个体 + 中心自身标记），直接送入 CNN 编码器。
     - `cnn_channels` / `cnn_feature_dim`：控制卷积层深度与编码维度。
     - `rnn_hidden_dim` / `rnn_type`：启用 GRU（默认）或 LSTM 作为短期记忆，隐状态会作为 Agent 侧的 `memory` 保存到 checkpoint，旧存档缺失该字段时会自动补零。
     - `hidden`：当 `use_cnn` 关闭时回退的 MLP 隐藏层规模。
   - `training`：并行环境数、回合步数、保存间隔等训练相关参数（示例：`num_envs: 8`, `save_interval: 100`, `checkpoint_dir: checkpoints/phase0`）。
     - `rollout_steps` 是本次运行的环境总步数；`n_steps` 是每次 PPO 更新最多收集的连续轨迹长度。
     - `gamma` / `gae_lambda` 控制多步回报与 GAE；轨迹末端使用价值网络 bootstrap，死亡、出生和槽位复用会切断回报。
     - `ppo_clip` 控制策略比率及价值裁剪，`ppo_epochs` 控制重复优化轮数，`ppo_minibatch_steps` 是连续时间小批长度及 TBPTT 长度。
     - 可用 `learning_rate` 调整 Adam 学习率（默认 `0.0001`），`max_grad_norm` 控制梯度裁剪。`entropy_coef` 与 `action_noise` 控制探索。
     若需要调试模型的动作采样，可开启 `print_actions: true` 查看每步动作编号与含义。
   - `action_rewards`：行为基础奖励，可为正/负（示例：`MOVE: 0.01`, `ATTACK: -0.01`）。
   - `rewards`：生存/进食奖励与食物感知奖励开关。
     - `food_reward_min` / `food_reward_max`：成功进食奖励的下限/上限；按进食前能量计算，能量越低奖励越高。
     - `food_reward_mode`：`linear` 为线性变化；`log` 会让奖励在能量接近上限时更集中地跌向最低值。
     - `food_reward_coefficient`：`log` 曲线强度，值越大，末端跌落越集中。旧模型若只有 `food_reward`，仍按固定奖励兼容。
     - `enable_proximity_reward`：是否启用视野内食物奖励（默认开启）。
     - `see_food_reward` 是每个可见食物的基础奖励：所有食物分别按距离衰减后累加，数量越多总奖励越高；脚下食物使用 `stand_on_food_reward` 替代该格普通奖励。
     - `vision_decay_mode` 与 `vision_decay_coefficient`：控制距离衰减；`linear` 模式在半径处衰减到 `max(0, 1 - coeff)`，`log` 模式使用 `1 / (1 + coeff * log(1+d))` 更平滑。
       例如感知半径为 3、`coeff=1.0` 时，线性衰减在距离 3 处约为 `0`，而对数衰减在距离 3 处仍保留约三分之一的奖励。
   - 阶段开关：各阶段配置文件通过布尔开关控制功能模块，例如 `use_reproduction`、`use_combat`、`use_communication`、`use_terraforming`。

   **如何自定义**：
    - 复制默认配置：`cp config/default.yaml config/my_exp.yaml`。
    - 按需修改上述字段；未修改的字段会沿用默认值。
    - 运行时指定：`python -m scripts.train_phase0 --config config/my_exp.yaml`。

## CNN + RNN 策略模式
- 默认 `model.use_cnn: true`，环境会为每个体在 GPU 上一次性裁剪局部网格 patch，shape 为 `(num_envs, agents_per_env, C, H, W)`，其中 `C` 包含墙/可建造区/食物/毒素/资源/其他个体以及中心“自身”指示。
- 策略网络改为 `CNNRecurrentPolicy`：卷积编码 -> GRU/LSTM 短期记忆 -> 动作 logits 与价值输出，兼容多环境、多智能体同时运行。
- Agent 侧新增 `memory`（以及 LSTM 时的 `memory_cell`）字段，reset 时自动清零，checkpoint 会连同地图与优化器一起保存/恢复；旧存档缺少该字段时自动补零以避免维度错误。
- 训练器采用 n-step rollout + GAE + PPO；更新时按连续时间块重新展开 RNN，`ppo_minibatch_steps` 同时决定 TBPTT 长度。死亡、出生和槽位复用会清断对应隐状态，避免把不同生命的经验串在一起。
- 若需要回退到旧的 MLP，占位策略，可在 YAML 中将 `model.use_cnn` 设为 `false`，其余训练入口与参数保持不变。

4. **常见问题排查**
   - `ModuleNotFoundError: No module named 'torch'`：确认已在当前虚拟环境中执行 `pip install torch pyyaml`。
   - CUDA 未被使用：检查 `torch.cuda.is_available()` 是否为 True，如否则会回退到 CPU。

5. **自定义地图加载与编辑**
   - 在配置 `world.map_file` 中指定十六进制地图文件；确保 `height/width` 与文件中单元数量一致（每个单元两个十六进制字符，长度 = `2*H*W`）。
   - 使用命令行编辑器快速制作地图：
     ```bash
     # 从空白 16x16 地图开始编辑，保存到 maps/custom_xxx.hex
     python -m scripts.map_editor --width 16 --height 16

     # 基于已有地图微调
     python -m scripts.map_editor --input maps/default_map.hex --width 8 --height 8 --output maps/my_map.hex

     # 打开 matplotlib 图形界面，点击涂抹/按 n 切换笔刷类型
     python -m scripts.map_editor --width 16 --height 16 --gui
     ```

6. **模型与环境 checkpoint / 断点续推**
   - 推荐每个模型/实验单独建目录：`model/<model_name>/`（如上文 Demo）。目录下需要有一份 `<model_name>.yaml` 作为训练/回放唯一入口。
   - 训练脚本新增参数：
     * `--save-interval`：每隔多少步保存模型与完整存档（默认读取配置中的 `training.save_interval`）。
     * `--checkpoint-dir`：保存目录（默认指向 `model/<name>/checkpoint`）。
     * `--model-dir` / `--model-name`：统一管理模型目录，便于续训或共享。
     * `--resume-from`：从 checkpoint 继续推演（恢复地图、agent 状态、模型、优化器与步数）；可传新三件套中的任一文件，也兼容旧 `full_step_*.pt`。
     * `--load-model`：仅加载模型权重，在新地图或新实验上测试。
     * 启动时会打印模型目录、配置文件、checkpoint 目录以及实际采用的 checkpoint 完整路径，便于确认输入输出位置。
   - 示例：
     ```bash
     # 从最新 checkpoint 直接续训（自动识别新三件套及旧 full 文件）
     python -m scripts.train_phase0 --model-dir model/<model_name>

     # 显式指定某个存档继续
     python -m scripts.train_phase0 --resume-from "model/<model_name>/checkpoint/#20_env_step_200.pth"

     # 从其他模型加载
     python -m scripts.train_phase0 --resume-from /<model_name>/checkpoint/full_step_200.pt
     
     # 加载他人分享的模型只需要把模型文件夹复制到自己的model/下就可以使用--model-dir调用了
     ```

   - 新 checkpoint 使用 `env_step_<n>.pth`、`model_step_<n>.pth`、`optim_step_<n>.pth` 三件套：环境/Agent、模型、优化器各保存一次，不再生成重复包含模型与优化器的 `full_step_*.pt`。三文件需放在同一目录，可直接用于“继续推演”或模型回滚。
   - 训练中的 log/checkpoint/console 文件暂用 `无效_<临时运行标识>_...`；本次实际推进超过 1024 step 并写入计数 JSON 后，会统一原子改名为 `#<有效模拟编号>_...`，最终文件名不再保留时间戳。自动续训会忽略仍带 `无效_` 的 checkpoint。
   - 每次保存前会估算模型、优化器和环境三件套的完整体积，并结合 `checkpoint_size_safety_factor` 与 `checkpoint_min_free_gb` 检查目标盘剩余空间。空间不足时不会写入任何 PT 临时文件，并立即停止训练；写完后还会验证三个 ZIP central directory 全部完整才落位。
   - checkpoint 同时保存 Python、NumPy、PyTorch CPU、全部 CUDA 设备及环境独立生成器的 RNG 状态；同设备拓扑续训时会接续保存点之后的随机序列。旧 checkpoint 缺少 RNG 状态时仍可加载，但会提示无法做到随机序列完全连续。
   - 自动续训会先检查 PyTorch ZIP 目录结构；保存中断产生的残缺文件会被警告并跳过，再回退到最近的完整 checkpoint。
   - 旧 `full_step_*.pt` 仍可直接续训。若旧目录同时已有 `model_step_`、`optim_step_`，可迁移环境状态并在校验后删除冗余 full 文件：
     ```bash
     # 先预览
     python -m scripts.compact_checkpoints model/<model_name>/checkpoint --dry-run

     # 迁移、校验并删除旧 full 文件；不加 --delete-full 则保留旧文件
     python -m scripts.compact_checkpoints model/<model_name>/checkpoint --delete-full
     ```

7. **实时渲染、日志与回放**
   - `logging.realtime_render: true` 时，环境会用 matplotlib 绘制首个并行环境的地图与 agent 位置（参考 `map_editor` 的配色，需安装 matplotlib）。
    - `logging.save_dir` 与 `logging.snapshot_interval` 控制回放日志写入；可调用 `protolife.replay.playback("model/<name>")`，或运行 `python -m scripts.visualize_replay --log-target model/<name>`。
    - 普通训练最终会生成 `#N_map.log`、`#N_agents.jsonl` 和 `#N_console.log`，正式文件名不再保留时间戳；不足 1025 step 的文件使用 `无效_<随机运行标识>_` 临时前缀，既避免覆盖也不携带时间戳。前两者记录回放数据，console 文件复制本次训练阶段的终端输出。Profiler 模式不生成这些训练日志。
    - 本次实际推进超过 1024 step 时，会更新项目根目录 `Simulation_counting.json`：顶层 `effective_simulation_count` 是权威累计值，`simulations` 保存模型名、训练类型、续训来源、时间、起止 step、运行步数和停止原因等结构化记录。可以直接手工修正顶层累计值，下一次会从修正值继续累加。
    - 第一次生成 JSON 时会只读迁移旧 `Simulation_counting.txt`，优先采用其中最大的 `累计有效模拟` 或 `有效模拟 #N`，不会再按文本行数把历史基线重置；旧 TXT 不修改、不删除。
    - 每条有效模拟记录还包含最终训练产物的绝对路径；结合 `resume_from` 与文件名前缀 `#N_`，可以还原模型的续训分支/平行世界结构。
    - 清理所有短跑、崩溃等无效训练产物：
      ```bash
      # 先预览
      python -m scripts.clean_invalid_training_files --dry-run

      # 默认递归清理 model；外部 checkpoint 可把根目录作为位置参数传入
      python -m scripts.clean_invalid_training_files
      python -m scripts.clean_invalid_training_files K:/Proto_Digital_Life
      ```
    - 回放：
      1. 进入对应的模型目录（例如 `model/demo/`），确认 `<model_name>.yaml`、`checkpoint/`、`log/` 存在。
      2. 回放：在'项目根目录'执行
         ```bash
         # 默认保留多日志交互选择，并从头播放
         python -m scripts.visualize_replay --log-target model/<model_name>

         # 仅在显式传入 --latest 时选创建时间最新的日志，并从 step 8000 开始
         python -m scripts.visualize_replay --log-target model/<model_name> --latest --start-step 8000
         ```

         或在代码中调用 `protolife.replay.playback`。

8. **个体数量曲线与实时监控**
   - 分文件绘图并监控文件变化：
     ```bash
     python scripts/plot_agent_count_v1.py model/test_CNN/log/旧日志_agents.jsonl model/test_CNN/log/新日志_agents.jsonl
     ```
     可以传入任意数量的 JSONL；每个文件使用与背景保持高对比度的独立颜色，重叠 step 会同时保留，不再互相覆盖。X/Y 均从 0 开始，Y 上限始终为实际最大值 + 2；X 主刻度间隔会随缩放自动选择 `2^n`。滚轮缩放，双击或按 `R` 复位；文件每 0.5 秒刷新，回到最右端后继续跟随最新 step。省略参数时进入逐行输入模式。
   - 实时监控正在写入的日志：
     ```bash
     python scripts/watch_agent_count_live.py model/test_CNN/log/旧日志_agents.jsonl model/test_CNN/log/当前日志_agents.jsonl
     ```
     文件变化后会自动刷新。鼠标左键拖动查看历史，滚轮缩放 X 轴，按 `f` 或 `r` 恢复跟随最新 step；省略参数同样进入逐行输入模式。

9. **PyTorch Profiler 性能调查**

   Profiler 只在显式传入 `--profiler` 时延迟导入和启动；普通训练不会初始化 Kineto，热循环只保留一次 `None` 判断。训练进度行同时输出 `agent_count`，便于把吞吐变化与当前存活个体数对应起来。

   ```bash
   # 使用默认 config/PyTorch_Profiler.yaml，其余训练参数照常传入
   python -m scripts.train_phase0 --model-dir model/<model_name> --config model/<model_name>/<model_name>.yaml --profiler

   # 使用单独的 Profiler 配置，便于做不同介入强度的对照实验
   python -m scripts.train_phase0 --model-dir model/<model_name> --config model/<model_name>/<model_name>.yaml --profiler config/My_Profiler.yaml
   ```

   - `test_steps` 控制本次测试总步数，临时覆盖训练配置的 `rollout_steps`；`skip_first` 丢弃前 N 步，之后由 `wait`、`warmup`、`active`、`repeat` 控制采集窗口。
   - `intensity` 可选 `light`、`balanced`、`deep`；也可逐项设置 shape、内存、调用栈、FLOPs 和模块信息。采集越深，扰动越大，建议先用 `light` 找主要热点。
   - Profiler 模式强制不保存训练 log/checkpoint，也不计入有效模拟，避免数 GB I/O 污染性能结果；只写入性能调查报告。
   - 报告默认写入 `profiler_reports/<model>_<timestamp>/`，包括 `summary.txt`、逐周期算子表、Chrome trace、`manifest.json` 和含模型/config/吞吐信息的 `metadata.json`，可用于不同模型与配置之间的对照。
   - 独立 CNN 会根据每一步的 alive/valid mask 只执行当前有效大脑，PPO 复用采样时缓存的有效索引；视野食物奖励改为局部坐标采样，避免整图 `unfold`。在 `test_CNN_NS_TBPTT` 同一 checkpoint 的 64 步实测中，吞吐由 `0.763` 提升到 `2.794 step/s`（约 `3.66×`）。

本 README 为概览，详细设计思路请参考源码中的中文注释，后续可在此基础上逐步补全能量代谢、战斗、通信等真实逻辑。
